...
const StartupProfile = () => {
  const { id } = useParams();
  const startupData = resolveStartupProfile(id);
      ...
      <main className="container mx-auto px-4 pt-20 pb-12">
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center space-y-4 md:space-y-0 md:space-x-6">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center space-x-4 text-sm text-muted-foreground mb-4">
                    <div className="flex items-center gap-1"><MapPin className="w-4 h-4" />{startupData.location}</div>
                    <div className="flex items-center gap-1"><Users className="w-4 h-4" />{startupData.team} team members</div>
                    <div className="flex items-center gap-1"><Calendar className="w-4 h-4" />{startupData.stage}</div>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <Card className="bg-card-gradient border-border">
              <CardHeader><CardTitle>About {startupData.name}</CardTitle></CardHeader>
              <CardContent><p className="text-muted-foreground">{startupData.longDescription}</p></CardContent>
            </Card>
          ...
          <div className="space-y-6">
            <Card className="bg-card-gradient border-border">
              ...
              <CardContent className="space-y-4">
                ...
                {startupData.coFounders.length > 0 && (
                  <div>
                    <h4 className="font-medium">Co-founders</h4>
                    <div className="space-y-1">
                      {startupData.coFounders.map((c, i) => (
            ...
            <Card className="bg-card-gradient border-border">
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center space-x-2"><Mail className="h-4 w-4 text-muted-foreground" /><span className="text-sm">{startupData.email}</span></div>
                <div className="flex items-center space-x-2"><Phone className="h-4 w-4 text-muted-foreground" /><span className="text-sm">{startupData.phone}</span></div>
                <div className="flex items-center space-x-2"><Globe className="h-4 w-4 text-muted-foreground" /><span className="text-sm">{startupData.website}</span></div>
              </CardContent>
            </Card>
          </div>
        </div>
